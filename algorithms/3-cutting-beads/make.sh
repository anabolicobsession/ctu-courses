...
EXEC="cmake-build-debug/hw3"
TEST_PREFIX="$TEST_DIR/$TEST_IN_DIR_PREFIX"

ARG_MAKE="make"

ARG_ZIP="zip"
ZIP_TARGET="alg03.tgz"
ZIP_FILES=("main.cpp")
...
ARG_HELP="help"
HELP_TEXT="Help script for testing programs, removing files or zip
test        run all tests
test N      run test number N
test N M    run tests in range [N,M], where N <= M
make        use Makefile to compile binary program
help        print help"
