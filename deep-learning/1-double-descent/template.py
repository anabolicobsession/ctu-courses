...
class MyNet:
    ...
    def score(self, x: np.array) -> np.array:
        ...
        *
        ...
        """
        a1 = relu(self.W1 @ x.T + self.b1.reshape(-1, 1))
        z2 = self.W2 @ a1 + self.b2.reshape(-1, 1)
        return z2.flatten()
    ...
    def train(self, train_data, lambda_=1e-5):
        """
        ...
        *
        ...
        a1 = relu(self.W1 @ x.T + self.b1.reshape(-1, 1))
        a1 = np.concatenate([np.ones((1, a1.shape[1])), a1], axis=0).T
        theta = np.linalg.inv(a1.T @ a1 + lambda_ * np.eye(a1.shape[1])) @ a1.T @ y.reshape(-1, 1)

        self.b2 = theta[0:1].reshape(-1)
        self.W2 = theta[1:].T
