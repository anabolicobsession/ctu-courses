...
def get_kernel(Xi, Xj, options):
    ...
    if options['kernel'] == 'linear':
        ...
        return np.power(1 + Xi.T @ Xj, options['d'])
    elif options['kernel'] == 'rbf':
        return np.exp(-np.sum(np.square(np.expand_dims(Xi, axis=-1) - np.expand_dims(Xj, axis=-2)), axis=0) / (2 * options['sigma'] ** 2))
    else:
        raise ValueError('invalid kernel name')


def svm(X, y, C, options, eps=1e-10):
    """
    ...
    ub = np.repeat(C, n)
    a = y
    alpha, _, _, finished = gsmo(H, f, a, lb=lb, ub=ub, verb=options['verb'], t_max=options['t_max'])
    sv_indexes = alpha > eps

    if finished:
        return {
            'sv': X[:, sv_indexes],
            'y': y[sv_indexes],
            'alpha': alpha[sv_indexes],
            'options': options,
            'b': compute_bias(K, y, alpha, C),
            'fun': classif_svm
        }
    else:
        raise Exception('gsmo has failed')
...
def classif_svm(X, model):
    ...
    """
    f = np.sum(model['alpha'].reshape(1, -1) * model['y'].reshape(1, -1) * get_kernel(X, model['sv'], model['options']), axis=-1) + model['b']
    classif = np.ones(len(f))
    classif[f <= 0] = -1
    return classif
...
def svm_crossvalidation(itrn, itst, X, y, C, options):
    ...
    for i in range(n_folds):
        model = svm(X[:, itrn[i]], y[itrn[i]], C, options)
        error.append(np.sum(y[itst[i]] != classif_svm(X[:, itst[i]], model)) / len(y[itst[i]]))
...
def compute_measurements_2d(data, normalization=None):
    ...
    a = np.sum(X[:, :half_of(w)], axis=(0, 1)).astype(int) - np.sum(X[:, half_of(w):], axis=(0, 1))
    b = np.sum(X[:half_of(h)], axis=(0, 1)).astype(int) - np.sum(X[half_of(h):], axis=(0, 1))

    if normalization is None:
        normalization = {'a_mean': np.mean(a), 'a_std': np.std(a), 'b_mean': np.mean(b), 'b_std': np.std(b)}

    def normalize(x, mean, dev):
        return (x - mean) / dev

    a = normalize(a, normalization['a_mean'], normalization['a_std'])
    b = normalize(b, normalization['b_mean'], normalization['b_std'])

    return np.vstack([a, b]), data['labels'], normalization
