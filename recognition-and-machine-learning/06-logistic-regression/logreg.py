...
def logistic_loss(X, y, w):
    ...
    exponent = - y * (w.reshape(1, -1) @ X).ravel()
    E = np.sum(np.log(1 + np.exp(exponent))) / len(y)
    return float(E)
...
def logistic_loss_gradient(X, y, w):
    ...
    """
    a = 1 / (1 + np.exp(y * np.squeeze(w.reshape(1, -1) @ X)))
    g = - np.sum(np.reshape(a * y, (1, -1)) * X, axis=1) / len(y)
    return g
...
def logistic_loss_gradient_descent(X, y, w_init, epsilon):
    ...
    """
    wt = np.array(np.expand_dims(w_init, axis=1))
    Et = np.array([logistic_loss(X, y, wt[:, 0])])
    learning_rate = 1.

    while wt.shape[1] <= 1 or np.linalg.norm(wt[:, -1] - wt[:, -2]) > epsilon:
        g = logistic_loss_gradient(X, y, wt[:, -1])

        while True:
            w_new = wt[:, -1] - learning_rate * g
        ...
        wt = np.hstack((wt, np.expand_dims(w_new, axis=1)))
        Et = np.append(Et, E_new)
...
def classify_images(X, w):
    ...
    """
    y = np.ones(X.shape[1])
    y[np.squeeze(w.reshape(1, -1) @ X) < 0] = -1
    return y
