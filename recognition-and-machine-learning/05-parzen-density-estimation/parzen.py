...
def my_parzen(x, x_trn, h):
    ...
    def parzen(x):
        s = 0
        for x_i in x_trn:
            s += norm.pdf(x - x_i, loc=0, scale=h)
        return s / len(x_trn)

    return np.apply_along_axis(parzen, 0, x)
...
def compute_Lh(itrn, itst, x, h):
    ...
    """
    log_likelihood = 0
    n_folds = min(len(itrn), len(itst))

    for train_indexes, test_indexes in zip(itrn, itst):
        estimates = my_parzen(x[test_indexes], x[train_indexes], h)
        log_likelihood += np.sum(np.log(estimates))

    return log_likelihood / n_folds
...
def classify_bayes_parzen(x_test, xA, xC, pA, pC, h_bestA, h_bestC):
    ...
    """
    xA_estimates = my_parzen(x_test, xA, h_bestA)
    xC_estimates = my_parzen(x_test, xC, h_bestC)
    labels = pA * xA_estimates < pC * xC_estimates
    return labels
