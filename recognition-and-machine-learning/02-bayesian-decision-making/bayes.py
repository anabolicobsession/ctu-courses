...
def bayes_risk_discrete(discrete_A, discrete_B, W, q):
    ...
    n = len(q)
    for x in range(n):
        R += discrete_A['Prob'][x] * discrete_A['Prior'] * W[0, q[x]]
        R += discrete_B['Prob'][x] * discrete_B['Prior'] * W[1, q[x]]
...
def find_strategy_discrete(discrete_A, discrete_B, W):
    ...
    """
    n = len(discrete_A['Prob'])

    def compute_loss(x, decision):
        return discrete_A['Prob'][x] * discrete_A['Prior'] * W[0, decision] + \
               discrete_B['Prob'][x] * discrete_B['Prior'] * W[1, decision]

    q = [0 if compute_loss(x, 0) <= compute_loss(x, 1) else 1 for x in range(n)]
...
def classify_discrete(measurements, q):
    ...
    """
    return q[np.int32(measurements + 10)]
